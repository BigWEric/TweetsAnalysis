...
public class getFromCouch {
	...
	public static void main(String args[]) {
		...
		CsvWriter wr1 = new CsvWriter(csvFile1Path.replace('/', '_'), ',',
				Charset.forName("SJIS"));
		CsvWriter wr2 = new CsvWriter(csvFile2Path.replace('/', '_'), ',',
				Charset.forName("SJIS"));
		...
		List<JsonElement> list = dbClient1.view(args[1]).groupLevel(0)
				.query(JsonElement.class);
				...
				if (entry.getKey().equals("key")) {
					...
					for (Entry<String, JsonElement> rawTweet : tweet
						...
						if (rawTweet.getKey().equals("geoLocation")) {
							csv[0] = rawTweet.getValue().toString()
									.substring(12, 32);
							csv[1] = rawTweet.getValue().toString()
									.substring(47, 57);
						}
						if (rawTweet.getKey().equals("text")) {
							Sen = SentimentAnalysis.findSentiment(rawTweet
									.getValue().toString());
		...
		try {
			FileWriter fw = new FileWriter(("SentiAnalysis_"
					+ args[1].replace('/', '_') + ".txt"), true);
			fw.write(args[1] + ":Positive " + p + " Negetive " + n
					+ " Neutral " + N + "\n");
			fw.close();
			wr1.close();
			wr2.close();
